...
*(Rendered as SVG because matplotlib is not installed in `out/pyenv`; the script
auto-writes `calibration_curve.png` instead when matplotlib is importable.)*
...
* **Train set** = the 36 remaining real questions **plus 16 synthetic exam
  ...
  scheduler (correct → *Good*, incorrect → *Again*; the engine counts
...
* Every outcome is drawn from a seeded per-topic **true ability** ∈ [0.30, 0.95]
  ...
  per-topic accuracy estimate and predicts *correct* iff that estimate ≥ 0.5.
...
* **What is real:** the FSRS retrievability formula and the per-topic
  performance-accuracy definition are the *actual* engine computations — eval 1
